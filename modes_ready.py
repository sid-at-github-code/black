...
if isinstance(SOURCE, int):
    ...
    print("Opening video file:", SOURCE)
    print("File exists:", os.path.exists(SOURCE))
    cap = cv2.VideoCapture(SOURCE)

if not cap.isOpened():
    raise SystemExit("❌ Cannot open input source")

# Optional: better webcam quality
if isinstance(SOURCE, int):
...
while True:
    ...
    for tid, tdata in list(tracks.items()):
        if (not tdata['counted']) and (tdata['consecutive'] >= CONFIRM_FRAMES):
            ...
            print("signal----------")
            print(f"Confirmed pothole id={tid} at frame {frame_idx}")
    ...
    cv2.imshow('Pothole Detector - Press Q to quit', frame)
...
print(f"Done. Frames: {frame_idx}, potholes: {unique_pothole_count}")