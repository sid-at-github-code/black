"""
pothole_server_video.py
=======================
FastAPI HTTP server + Pothole detection (VIDEO FILE only).
...
Run:
    python pothole_server_video.py
"""
...
# ─────────────────────────────────────────────
#  ML LOOP  (video file only)
# ─────────────────────────────────────────────
def run_ml_pipeline():
    global ml_running, unique_pothole_count

    print("\n[ML] ─────────────────────────────────────────")
    print(f"[ML] Opening VIDEO: {SOURCE}")

    if not os.path.isfile(SOURCE):
        print(f"[ML] ❌ File not found: {SOURCE}")
        ml_running = False
        return
    ...
    if not cap.isOpened():
        print(f"[ML] ❌ Cannot open video: {SOURCE}")
        ml_running = False
        return
    ...
    print(f"[ML] FPS          : {fps}")
    print(f"[ML] Total frames : {total_frames}")
    ...
    while ml_running:
        ...
        cv2.putText(frame, f"Potholes: {unique_pothole_count}  Frame: {progress}",
                    (10, 28), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)

        cv2.imshow('Pothole Detector [VIDEO] — Q to quit', frame)
...
app = FastAPI(title="Pothole IoT Server — VIDEO", lifespan=lifespan)
...
if __name__ == "__main__":
    print("=" * 55)
    print("  🛣️  Pothole IoT Server  [VIDEO MODE — HTTP]")
